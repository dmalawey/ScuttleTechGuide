...
- ![image scuttle acronym](img/intro_diagram1.jpg)
- ![image scuttle overlay](img/intro_diagram2.jpg)
- ![placeholder](img/img_placeHolder.jpg)


**Actuators**
SCUTTLE is designed as an architecture for developers to easily automate actuators.  One example is the spraymaster shown below (middle) that features a spraying mechanism with a cleaning product.  If a designer wanted a robot that performs cleaning, the job is simply to add an actuator and a driver for that actuator.  This project shows a linear actuator and a relay with a 3D printed assembly that holds the spray can.  In this project, the existing computer and power supply are connected to the new actuator system to give power and computing.  The hardware modules and software modules are pre-built so you can copy and paste most of the engineering effort to support brand new projects.  You can [download spraymaster design](https://grabcad.com/library/spraymaster-1) freely.
